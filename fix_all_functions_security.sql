-- =====================================================
-- FIX: Sécuriser TOUTES les fonctions vulnérables
-- =====================================================
-- Ce script corrige toutes les fonctions sans search_path fixé
-- détectées par l'audit de sécurité
-- =====================================================
...
AS $$
BEGIN
    -- Décrémenter le stock du matériau
...
AS $$
BEGIN
    -- Restaurer le stock du matériau
    UPDATE public.materials
...
-- Recréer les triggers (pour toutes les tables qui l'utilisent)
...
BEGIN
    ...
    IF vulnerable_count = 0 THEN
        RAISE NOTICE '✅ SUCCÈS ! Toutes les fonctions sont maintenant sécurisées !';
        RAISE NOTICE '✅ 8 fonctions corrigées avec search_path fixé';
        RAISE NOTICE '✅ SECURITY INVOKER activé sur toutes les fonctions';
    ELSE
        RAISE WARNING '⚠️ ATTENTION : % fonction(s) sont encore vulnérables', vulnerable_count;
    END IF;
END $$;
